...
const fetchWaitingProfessionals = async (
  ...
  try {
    ...
    const response = await axiosInstance.get(`/professionaluser/active?${params}`);
...
export default function ActiveProfessionalsTable() {
  ...
  return `${firstName} ${middleName} ${lastName}`.trim() || "N/A";
    ...
    { accessorKey: "City", header: "City",
          cell: info => {
         const data = info.getValue() as string | undefined;
  ...
  const isValidUrl = typeof value === "string" && value.startsWith("https");
        return (
          <Image
            src={isValidUrl ? value : "/images/defaultlogo.png"}
            alt="headshot"
            width={32}
            height={32}
      ...
      cell: (info) => {
        ...
        return (
          <button type="button" className="bg-primary hover:bg-red-700  transition-colors duration-300 p-2 rounded-full">
            <Link href={`/active-professionals/${row.Phone}`}>
              <svg
...
/>
          ...
          }} className=" py-2 px-3 bg-red-800 text-white rounded flex items-center gap-2">
