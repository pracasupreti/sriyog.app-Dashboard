...
try{
   const page = parseInt(req.query.page) || 1;
    ...
    const totalCount = await ProfessionalUser.countDocuments(searchQuery);
    const totalPages = Math.ceil(totalCount / limit);

    const joinForms = await ProfessionalUser.find(searchQuery)
      .skip(skip)
      .limit(limit)
      .sort({ createdAt: -1 });
...
export const getActiveProfessionalsById = async (req, res) => {
  try {
    const data= Number(req.params.id);
		const joinForm = await ProfessionalUser.findOne({Phone:data});
		if (!joinForm) {
			return res.status(404).json({ error: 'User not found' });
		}
		res.json(joinForm);
